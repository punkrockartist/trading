...
def auth_ws(svr="prod", product=_cfg["my_prod"]):
    ...
    _base_headers_ws["appkey"] = _TRENV.my_app
    _base_headers_ws["appsecret"] = _TRENV.my_sec
...
class KISWebSocket:
    ...
    async def __runner(self):
        ...
        # WebSocket URL 변환
        # KIS API는 ws:// 프로토콜을 사용 (wss:// 아님)
        base_url = getTREnv().my_url_ws
        if base_url.startswith("https://"):
            # https://를 ws://로 변환 (포트도 변경: 9443 -> 21000, 29443 -> 31000)
            if ":9443" in base_url:
                base_url = base_url.replace("https://openapi.koreainvestment.com:9443", "ws://ops.koreainvestment.com:21000")
            elif ":29443" in base_url:
                base_url = base_url.replace("https://openapivts.koreainvestment.com:29443", "ws://ops.koreainvestment.com:31000")
            else:
                base_url = base_url.replace("https://", "ws://", 1)
        elif base_url.startswith("http://"):
            base_url = base_url.replace("http://", "ws://", 1)
        elif not base_url.startswith(("wss://", "ws://")):
            # 프로토콜이 없으면 ws:// 추가
            base_url = f"ws://{base_url}"
        
        # WebSocket 연결에 필요한 헤더 가져오기
        headers_ws = _getBaseHeader_ws()
        
        url = f"{base_url}{self.api_url}"
        
        # 모든 인증 정보를 헤더로 전달 (approval_key, appkey, appsecret)
        additional_headers = {}
        if "approval_key" in headers_ws:
            additional_headers["approval_key"] = headers_ws["approval_key"]
        if "appkey" in headers_ws:
            additional_headers["appkey"] = headers_ws["appkey"]
        if "appsecret" in headers_ws:
            additional_headers["appsecret"] = headers_ws["appsecret"]

        # 디버깅: 연결 정보 출력
        print(f"[WebSocket 연결 시도] URL: {url}")
        print(f"[WebSocket 헤더 개수] {len(additional_headers)}")
        if "approval_key" in additional_headers:
            print(f"[approval_key] {additional_headers['approval_key'][:30]}...")
        if "appkey" in additional_headers:
            print(f"[appkey] {additional_headers['appkey'][:20]}...")

        while self.retry_count < self.max_retries:
            try:
                print(f"[연결 시도 {self.retry_count + 1}/{self.max_retries}]")
                # websockets 라이브러리에서 헤더 전달
